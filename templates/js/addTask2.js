...
function createTask() {
    ...
    if (category === 'Select task category') {
        document.getElementById('selectCategory').classList.remove('d-none');
    }
    else {
        setCreatedTask(category);
    }
}
 ...
 */
function setCreatedTask(category) {
    let title = document.getElementById('title').value;
    let description = document.getElementById('description').value;
    readColourFromCategory();
    let date = dateArray;
...
/**
 * sets information in newTask
 * @returns newTask
 */
function setNewTask(category, title, description, categoryColor, date) {
    let newTask = {
        'id': '',
        'title': title,
 ...
 */
function readColourFromCategory() {
    let readColourCategory = document.getElementById("category").childNodes[1];
